...
function gcSummary() {
  ...
  let key = 0;
  return Object.entries(system.gc)
    .map(([type, gc]) => {
      const count = gc[0];
      const total = gc[1];
      ...
      return [
        <div key={key++} className="gc-cat">
          <div className="gc-name">
            {type.replace(/ Generation$/, "")} ({count})
          </div>
          <span style={{ color: avgColor }}>{avg}ms</span> /{" "}
          <span style={{ color: rateColor }}>{rate}s</span>
        </div>,
      ];
    })
    .flat();
...
export default class SidebarTimingsInfo extends React.Component {
  constructor(props, ctx) {
    ...
    this.state = {
      dataReady: false,
      activeTab:
        (location.hash && location.hash.substring(1)) || this.props.activeTab, // TODO: Hash lookup/query string?
    };
    data.onReady(() => {
  ...
  render() {
    ...
    let minecraft = null;
    if (info.version.includes(" (MC: "))
      minecraft = info.version.split(" (MC: ")[1].split(")")[0];

    let server_id = data.timingsMaster.server;;

    let gc = data.timingsMaster.system.gc;

    return (
      <table className="sidebar-info">
        <tbody>
          <tr>
            <th className="fieldName" title="Uptime">
              <FA icon="hourglass-half" />
            </th>
            <td className="fieldValue" colSpan="4">
              {round(info.system.runtime / 60 / 60 / 1000, 2)}hr
            </td>
          </tr>
          <tr>
            <th className="fieldName" title="Version">
              <FA icon="code-branch" />
            </th>
            <td className="fieldValue" colSpan="4">
              {version}
          ...
          {minecraft ? (
            <tr>
              <th className="fieldName" title="Minecraft">
                <FA icon="cube" />
              </th>
              <td className="fieldValue" colSpan="4">
                Minecraft {minecraft}
          ...
          {gc ? (
            <tr>
              <th className="fieldName" title="GC">
                <FA icon="recycle" />
              </th>
              <td className="fieldValue" colSpan="4">
                {gcSummary()}
